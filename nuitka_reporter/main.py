...
from ._version import __version__ as _reporter_version
from ._types import NumberLike
from .plot import size, time
from .plot.plotter import Plotter
from .experiments import dependency_from_report
from .helpers import get_command_line, get_data_files, get_distributions, get_included, get_nuitka_version_raw, get_plugin_options, get_module_stats, has_nuitka_version_upgraded_report
...
def to_html(filename: str, export_filename: str = os.path.join(".", "index.html")):
    ...
    body_html = (
        ...
        f'<h2 class="mb-0">Nuitka Compilation Report <small>(Nuitka: v{get_nuitka_version_raw(filename)})</small></h2>'
    ...
    full_html = (
        ...
        f'{SWITCH_GRAPH_JS}'
        '<footer class="text-center text-muted py-3 mt-2 border-top">'
        f'<small>Generated by <a href="https://github.com/Bingdom/Nuitka-Compile-Report-Parser" class="text-muted">nuitka-reporter</a> v{_reporter_version}</small>'
        '</footer>'
        '</body>'
        '</html>'
    )
