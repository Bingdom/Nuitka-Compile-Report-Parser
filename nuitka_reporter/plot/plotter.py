...
def build_hierarchy_data(
    ...
    for root_module, submodules in sorted_modules:
        ...
        data.hover_text.append(
            f"{root_module}<br>{value_formatter(root_total)}")
        ...
        for path in sorted(all_paths - {root_module}, key=lambda x: (x.count("."), x)):
            ...
            data.hover_text.append(
                f"{path}<br>{value_formatter(total)}")
...
class Plotter():
    ...
    def __init__(self, file_path: str, module_parser: Callable[..., tuple[defaultdict[str, defaultdict[str, NumberLike]], NumberLike]], value_formatter: Callable[[NumberLike], str], title: str, xaxis_title: str, yaxis_title: str):
